...
class WebsiteController extends Controller
{
    ...
    function prepareWebsiteListInfo($listInfo, $errMsg)
    {
        //for create and update website

        $listInfo['name'] = strip_tags($listInfo['name']);
        $listInfo['url'] = addHttpToUrl($listInfo['url']);

        $listInfo['statusVal'] = isset($listInfo['status']) ? intval($listInfo['status']) : 1;
        $listInfo['ya_host_id'] = isset($listInfo['ya_host_id']) ? $listInfo['ya_host_id'] : '';
        $listInfo['ya_mirror_id'] = isset($listInfo['ya_mirror_id']) ? $listInfo['ya_mirror_id'] : '';
    ...
    function createWebsite($listInfo, $apiCall = false)
    {
        ...
        $errMsg = [];
        $listInfo = $this->prepareWebsiteListInfo($listInfo, $errMsg);
            ...
            if (!$this->__checkName($listInfo['name'], $userId)) {
                if (!$this->__checkWebsiteUrl($listInfo['url'])) {
    				...
    				values('" . addslashes($listInfo['name']) . "','"
                        ...
                        . addslashes($listInfo['keywords']) . "', $userId,"
                        . $listInfo['statusVal'] . ",'"
                        . addslashes($listInfo['ya_host_id']) . "','"
                        . addslashes($listInfo['ya_mirror_id']) . "' "
                        . ")";
                    $this->db->query($sql);

                    $ret['status'] = true;
                    $ret['msg'] = 'Successfully created website';
                    // if api call
                    if ($apiCall) {
                        return $ret;
                    ...
                    $ret['msg'] = formatErrorMsg($this->spTextWeb['Website already exist.']);
                }
            } else {
        ...
        if (!$this->validate->flagErr) {
            ...
            if (!$this->validate->flagErr) {
                ...
                $sql = "update websites set
						...
						status = '" . addslashes($listInfo['statusVal']) . "',
						keywords = '" . addslashes($listInfo['keywords']) . "',
						ya_host_id = '" . addslashes($listInfo['ya_host_id']) . "',
						ya_mirror_id = '" . addslashes($listInfo['ya_mirror_id']) . "'
