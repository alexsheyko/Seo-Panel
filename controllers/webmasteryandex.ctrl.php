...
class WebYandexController extends YandexWebAPIController
{

    var $rowLimit = 5000;
    var $sourceList = array('yandex');
    var $colList = array();
    ...
    function getAllSites($userId)
    {
        ...
        try {
            ...
            $service = new Yandex_Service_Webmasters($client->getAccessToken());
            $resultList = $service->listSites();
            //$result['status'] = false;
            //$result['msg'] = $resultList['msg'];
    ...
    function storeWebsiteAnalytics($websiteId, $reportDate, $source = "google")
    {
        ...
        if ($result['status']) {
            ...
            foreach ($result['resultList'] as $reportInfo) {
                ...
                if ($keywordId == 0) {
                    $keywordId = $this->__generateKeywordId($websiteInfo['id'], $keywordName);
                }
    ...
    function viewKeywordSearchGraphReports($searchInfo = '')
    {
        ...
        if (!empty($searchInfo['attr_type'])) {
            ...
            if ($searchInfo['attr_type'] == 'average_position') {
